...
sty_path = opt.sty
tar_path = opt.tar
sty_list = collect_image_files(sty_path)
tar_list = collect_image_files(tar_path)
if len(sty_list) != len(tar_list):
  print("The number of style and stylized images are not matched!!", f"sty: {len(sty_list)}, tar: {len(tar_list)}")
  exit()
hist_list = []
for sty, tar in tqdm(zip(sty_list, tar_list), total=len(sty_list)):
    ...
    input_image = image_loader(tar_fname)
    target_image = image_loader(sty_fname)
