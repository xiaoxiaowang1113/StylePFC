...
def download(url, ckpt_dir=None):
    ...
    if ckpt_dir is None:
        ckpt_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'weights')
    ...
    if not os.path.exists(ckpt_file):
        ...
        if total_size_in_bytes != 0 and progress_bar.n != total_size_in_bytes:
            ...
            if os.path.exists(ckpt_file_temp):
                # Preserve an interrupted download for inspection without leaving
                # a file that can be mistaken for a valid checkpoint.
                ...
                os.replace(
                    ckpt_file_temp,
                    os.path.join(quarantine_dir, os.path.basename(ckpt_file_temp)),
                )
        else:
