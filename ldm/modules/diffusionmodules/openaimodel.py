...
class UNetModel(nn.Module):
    ...
    def forward(self,
        ...
        for module in self.output_blocks:
            ...
            k_style_feature_key = f'output_block_{module_i}_self_attn_k_style'
            k_content_feature_key = f'output_block_{module_i}_self_attn_k_content'
            v_style_branch_feature_key = f'output_block_{module_i}_self_attn_v_style_branch'
            v_content_feature_key = f'output_block_{module_i}_self_attn_v_content'
            ...
            if injected_features is not None and k_style_feature_key in injected_features:
                self_attn_k_style_injected = injected_features[k_style_feature_key]

            if injected_features is not None and k_content_feature_key in injected_features:
                self_attn_k_content_injected = injected_features[k_content_feature_key]

            if injected_features is not None and v_style_branch_feature_key in injected_features:
                self_attn_v_style_branch_injected = injected_features[v_style_branch_feature_key]

            if injected_features is not None and v_content_feature_key in injected_features:
                self_attn_v_content_injected = injected_features[v_content_feature_key]
            ...
            if injected_features is not None and config_key in injected_features:
                # Copy the config so per-block tweaks never mutate the shared step payload in place.
                injection_config = dict(injected_features[config_key])
